-- View
CREATE OR REPLACE VIEW pencatatan_ternak_masuk_cdc AS
WITH cte_raw_parse AS (
  SELECT
    ...
    (old_data->>'jenis_mitra_pengirim')::VARCHAR(255) AS old_jenis_mitra_pengirim,
    ...
    (new_data->>'jenis_mitra_pengirim')::VARCHAR(255) AS new_jenis_mitra_pengirim,
  ...
  FROM log_cdc_prc
  WHERE table_name = 'pencatatan_ternak_masuk'
...
cte_op_update AS (
  SELECT
    ...
    new.tgl_pencatatan,
    new.id_peternak,
    new.jenis_mitra_pengirim AS jenis_mitra_pengirim,
