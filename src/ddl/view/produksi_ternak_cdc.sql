...
WITH cte_raw_parse AS (
  SELECT
    ...
    (new_data->>'tgl_produksi')::DATE AS new_tgl_produksi,
    (new_data->>'id_unit_ternak')::INT8 AS new_id_unit_ternak,
    (new_data->>'id_jenis_produk')::INT8 AS new_id_jenis_produk,
    (new_data->>'sumber_pasokan')::VARCHAR(255) AS new_sumber_pasokan,
  ...
  FROM log_cdc_prc
  WHERE table_name = 'produksi_ternak'
    AND is_processed IS FALSE
),
cte_op_insert AS (
  SELECT
    new_id AS id,
    new_tgl_produksi AS tgl_produksi,
    new_id_unit_ternak AS id_unit_ternak,
    new_id_jenis_produk AS id_jenis_produk,
    new_sumber_pasokan AS sumber_pasokan,
